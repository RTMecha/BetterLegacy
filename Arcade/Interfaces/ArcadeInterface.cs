...
namespace BetterLegacy.Arcade.Interfaces
{
    ...
    public class ArcadeInterface : BaseInterface
    {
        ...
        public IEnumerator RefreshOnlineLevels()
        {
            ...
            string query = LinkQuery.BuildQuery(currentTab == 0 ? AlephNetwork.LevelSearchURL : AlephNetwork.LevelCollectionSearchURL, search, page, sort, ascend);
                    ...
                    try
                    {
                        ...
                        if (jn["items"] != null)
                        {
                            ...
                            for (int i = 0; i < jn["items"].Count; i++)
                            {
                                ...
                                {
                                    CoroutineHelper.StartCoroutine(AlephNetwork.DownloadBytes($"{coverURL}{id}{FileFormat.JPG.Dot()}?r" + UnityRandom.Range(0, int.MaxValue),
                                        callback: bytes =>
                                        {
