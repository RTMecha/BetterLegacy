...
{
    /// <summary>
    /// Unity BPM Analyzer taken from https://github.com/WestHillApps/UniBpmAnalyzer (also found in newer versions of PA)
    /// </summary>
    public class UniBpmAnalyzer
    {
        ...
        static BpmMatchData[] bpmMatchDatas = new BpmMatchData[MAX_BPM - MIN_BPM + 1];
        ...
        public static IEnumerator IAnalyzeBPM(AudioClip clip, Action<int> result)
        {
            for (int i = 0; i < bpmMatchDatas.Length; i++)
            {
                bpmMatchDatas[i].match = 0f;
