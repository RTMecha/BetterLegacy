    ...
    public class ObjectConverter : Exists
    {
        ...
        RTBeatmapObject ToRuntimeObject(BeatmapObject beatmapObject)
        {
            ...
            VisualObject visual = shapeType switch
            {
                ...
                ShapeType.Polygon => new PolygonObject(visualObject, opacity, deco, isSolid, (int)beatmapObject.renderLayerType, beatmapObject.opacityCollision, (int)beatmapObject.gradientType, beatmapObject.gradientScale, beatmapObject.gradientRotation, (int)beatmapObject.colorBlendMode, beatmapObject.polygonShape, beatmapObject.particleSystemData),
                _ => new SolidObject(visualObject, opacity, deco, isSolid, (int)beatmapObject.renderLayerType, beatmapObject.opacityCollision, (int)beatmapObject.gradientType, beatmapObject.gradientScale, beatmapObject.gradientRotation, (int)beatmapObject.colorBlendMode, beatmapObject.particleSystemData),
            };
        ...
        #region Runtime Particles

        public bool SkipRuntimeParticles(BeatmapObject beatmapObject) => beatmapObject.objectType != ObjectType.Particles || SkipRuntimeObject(beatmapObject);

        public IEnumerable<IRTObject> ToRuntimeParticles(IEnumerable<BeatmapObject> beatmapObjects)
        {
            foreach (var beatmapObject in beatmapObjects)
            {
                if (SkipRuntimeParticles(beatmapObject))
                {
                    beatmapObject.runtimeParticles = null;
                    continue;
                }

                var runtimeParticles = ToRuntimeParticles(beatmapObject);

                if (runtimeParticles)
                    yield return runtimeParticles;
            }
        }

        public IRTObject ToIRuntimeParticles(BeatmapObject beatmapObject) => SkipRuntimeParticles(beatmapObject) ? null : ToRuntimeParticles(beatmapObject);

        RTParticles ToRuntimeParticles(BeatmapObject beatmapObject)
        {
            var runtimeParticles = new RTParticles(beatmapObject, runtimeLevel);
            beatmapObject.runtimeParticles = runtimeParticles;
            return runtimeParticles;
        }
        ...
        public static Vector3Keyframe DefaultScaleKeyframe => new Vector3Keyframe(0f, Vector3.one, Ease.Linear);
        ...
        public void UpdateCachedSequence(BeatmapObject beatmapObject, CachedSequences collection)
        {
            ...
            collection.ScaleSequence = GetVector2Sequence(beatmapObject, beatmapObject.events[1], DefaultScaleKeyframe);
        ...
        public static Sequence<Vector3> GetVector2Sequence(PAObjectBase obj, List<EventKeyframe> eventKeyframes, Vector3Keyframe defaultKeyframe) => new Sequence<Vector3>(GetVector2Keyframes(obj, eventKeyframes, defaultKeyframe));

        public static List<IKeyframe<Vector3>> GetVector2Keyframes(PAObjectBase obj, List<EventKeyframe> eventKeyframes, Vector3Keyframe defaultKeyframe)
        {
            ...
            foreach (var eventKeyframe in eventKeyframes)
            {
                ...
                keyframes.Add(new Vector3Keyframe(eventKeyframe.time, currentValue, Ease.GetEaseFunction(eventKeyframe.curve.ToString())));
                num++;
            }
