...
{
    public class RTPrefabObject : Exists
    {
        #region Core

        public RTPrefabObject(Prefab prefab, PrefabObject prefabObject)
        {
            Debug.Log($"{RTLevel.className}Loading prefab");
            ...
            AddPrefabToLevel(prefabObject, false);
            ...
            Debug.Log($"{RTLevel.className}Loaded {objects.Count} objects (original: {gameData.beatmapObjects.Count})");
        }
        ...
        public void Tick()
        {
            if (!RTLevel.UseNewUpdateMethod)
                CurrentTime = RTLevel.FixedTime;
            else
            {
                var smoothedTime = Mathf.SmoothDamp(previousAudioTime, RTLevel.FixedTime, ref audioTimeVelocity, 1.0f / 50.0f);
        ...
        public void Clear()
        {
            Debug.Log($"{RTLevel.className}Cleaning up level");
        ...
        public void RecacheAllSequences()
        {
            ...
            var expandedObjects = PrefabObject.expandedObjects;
            for (int i = 0; i < expandedObjects.Count; i++)
            {
                if (expandedObjects[i] is BeatmapObject beatmapObject)
                    RecacheSequences(beatmapObject, recursive: false);
            }
        }
        ...
        public void RecacheSequences(BeatmapObject beatmapObject, bool reinsert = true, bool updateParents = true, bool recursive = true)
        {
            if (!reinsert)
            {
                ...
                if (recursive)
                {
                    ...
                    for (int i = 0; i < expandedObjects.Count; i++)
                    {
                        var bm = expandedObjects[i] as BeatmapObject;
                        if (bm && bm.Parent == beatmapObject.id)
        ...
        public void UpdateHomingKeyframes()
        {
            ...
            {
                if (PrefabObject.expandedObjects[i] is not BeatmapObject beatmapObject)
                    return;
        ...
        public void AddPrefabToLevel(PrefabObject prefabObject, bool update = true, bool recalculate = true)
        {
            ...
            for (int i = 0; i < prefabObject.RepeatCount + 1; i++)
            {
                ...
                foreach (var beatmapObject in prefab.beatmapObjects)
                {
                    ...
                    {
                        Debug.LogError($"{RTLevel.className}Failed to set event speed.\n{ex}");
                    }
