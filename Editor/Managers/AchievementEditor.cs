    ...
    public class AchievementEditor : BaseManager<AchievementEditor, EditorManagerSettings>
    {
        ...
        public override void OnInit()
        {
            try
            {
                ...
                Popup = RTEditor.inst.GeneratePopup(EditorPopup.ACHIEVEMENTS_POPUP, "Select an Achievement", Vector2.zero, new Vector2(600f, 400f));
                Popup.InitTopElementsParent();
                Popup.InitReload(LoadAchievements);
                Popup.onRender = () =>
                {
                    if (AssetPack.TryReadFromFile("editor/ui/popups/achievements_popup.json", out string uiFile))
                    {
                        ...
                        if (jn["title"] != null)
                        {
                            Popup.title = jn["title"]["text"] != null ? jn["title"]["text"] : "Select an Achievement";
